...
const LeadFieldManager = ({ isOpen, onClose, onSave }) => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        getFieldConfig()
            .then(r => setFields((r.fields || []).map(f => ({ ...f, _open: false }))))
    ...
    const addField = () => setFields(prev => [...prev, {
        key: "", label: "", type: "text", required: false, options: [], order: prev.length, _open: true,
    }]);
    ...
    const updateOption = (fi, oi, patch) => {
        const opts = fields[fi].options.map((o, idx) => idx === oi ? { ...o, ...patch } : o);
    ...
    const handleSave = async () => {
        ...
        try {
            setSaving(true);
            const clean = fields.map(({ _open, ...rest }, i) => ({ ...rest, order: i }));
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
            ...
            <div className="relative w-full sm:max-w-lg max-h-[90vh] bg-white sm:rounded-2xl rounded-t-2xl shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-2">
                    ...
                    <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 mb-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {["Contact Number *", "Organisation *", "Address", "Contact Person", "Designation", "Cell Number", "Email", "Rooms", "Extra", "Status", "Assigned To"].map(f => (
                    ...
                    {loading ? (
                        ...
                        <>
                            {fields.length === 0 && (
                                <p className="text-xs text-gray-400 text-center py-4">No custom fields yet. Add one below.</p>
                            )}
                            {fields.map((field, i) => (
                                <div key={i} className="border border-gray-200 rounded-xl overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-2 px-3 py-2.5 bg-gray-50 cursor-pointer"
                                        ...
                                        <span className="text-[10px] text-gray-400 uppercase">{field.type}</span>
                                        ...
                                        <div className="px-3 py-3 space-y-2.5 border-t border-gray-100">
                                            <div className="grid grid-cols-2 gap-2">
                                                <div>
                                                    ...
                                                    <input value={field.label}
                                                        ...
                                                        placeholder="e.g. Budget" className={inp} />
                                                </div>
                                                <div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2">
                                                <div>
                                                    ...
                                                    <select value={field.type} onChange={e => update(i, { type: e.target.value })} className={inp}>
...
export default LeadFieldManager;
